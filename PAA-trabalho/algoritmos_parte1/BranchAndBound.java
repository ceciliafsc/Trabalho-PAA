    ...
    private void permutar(List<Peca> lista, int inicio) {
        ...
        for (int i = inicio; i < lista.size(); i++) { // troca todas as posições
            Collections.swap(lista, inicio, i);
             List<Placa> placas = montarPlacas(lista); //monta placa com lista atual
             double custo = calcularCustoTotal(placas);//calcula custo atual
                if(custo < min){ //se custo atual for menor que o minimo global, continua
